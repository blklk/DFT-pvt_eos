...
@dataclass # this is a decorator that allows you to add special methods to classes
class RegexPatterns:
...
regex = RegexPatterns()
# RegexPatterns is a dataclass that contains all the regular expressions that are used in the code
...
def fit_eos(
    ...
    logging.info(
        f"Initial guesses: V0={initial_guesses[0]}, E0={initial_guesses[1]}, K0={initial_guesses[2]}, Kp0={initial_guesses[3]}")

    if fixed_Kp0 is not None:
        def bm3_eos_energy_fixed_Kp0(V, V0, E0, K0):
            return bm3_eos_energy(V, V0, E0, K0, fixed_Kp0)

        popt, pcov = curve_fit(
            bm3_eos_energy_fixed_Kp0,
            V,
            F,
            p0=initial_guesses[:3],  # Only fitting V0, E0, and K0
            maxfev=10000
        )
        #print("in fixed case")
        #print(popt)
        popt = np.append(popt, fixed_Kp0)  # Append fixed Kp0 to the results
        #print(popt)
        perr = np.sqrt(np.diag(pcov))
        perr = np.append(perr, 0.0)  # Error for fixed Kp0 is zero
    else:
        # If Kp0 is not fixed, fit all four parameters
        popt, pcov = curve_fit(
            bm3_eos_energy,
            V,
            F,
        ...
        #print("in not fixed case")
        #print(popt)
        perr = np.sqrt(np.diag(pcov))

    logging.info(f"Fitted values: V0={popt[0]}, E0={popt[1]}, K0={popt[2]}, Kp0={popt[3]}")

    parameters = BM3EOSParameters(
        V0=popt[0],
        E0=popt[1],
        K0=popt[2],
        Kp0=popt[3],  # This should now be fixed_Kp0
        V0_err=perr[0],
        E0_err=perr[1],
        K0_err=perr[2],
        Kp0_err=perr[3]
    ...
    if verbose:
        #if fixed_Kp0 is not None:
       #    logging.info(f"Fitted 3rd order Birch-Murnaghan EOS parameters with Kp0 fixed at {fixed_Kp0}:")
       # else:
...
def fit_pressure_eos(
    ...
    """Fit parameters of a 3rd order BM EOS using pressure data, optionally fixing Kp0."""
    if initial_guesses is None:
        initial_guesses = [
            np.mean(V),  # V0_guess
            240.0 / 160.218,  # K0_guess in eV.A**3
    ...
    # If Kp0 is fixed, define a wrapper function that only fits V0 and K0
    if fixed_Kp0 is not None:
        def bm3_eos_pressure_fixed_Kp0(V, V0, K0):
            return bm3_eos_pressure(V, V0, K0, fixed_Kp0)

        popt, pcov = curve_fit(
            bm3_eos_pressure_fixed_Kp0,
            V,
            P / 160.218,  # Convert GPa to eV.A**3
            p0=initial_guesses[:2],  # We only fit V0 and K0
            maxfev=10000
        )
        print(popt)
        popt = np.append(popt, fixed_Kp0)  # Add the fixed Kp0 to the result
        perr = np.sqrt(np.diag(pcov))
        perr = np.append(perr, 0.0)  # Error for fixed Kp0 is zero
    else:
        # If Kp0 is not fixed, fit all three parameters
        popt, pcov = curve_fit(
            bm3_eos_pressure,
            V,
            P / 160.218,  # Convert GPa to eV.A**3
...
Callable[[float], float], Callable[[float], float]]:
    ...
    # Fit V0, E0, K0 as functions of temperature
    for param_name, data in zip(['V0', 'E0', 'K0', 'Kp0'], [V0s, E0s, K0s, Kp0s]):
        if param_name == 'Kp0' and fixed_Kp0 is not None:
            # Handle Kp0: If fixed, return a constant array
            fitted_funcs[param_name] = lambda t, fixed_Kp0=fixed_Kp0: np.full_like(t, fixed_Kp0)
        else:
            popt, _ = curve_fit(quintic_function, Ts, data, maxfev=10000)
            fitted_funcs[param_name] = lambda t, popt=popt: quintic_function(t, *popt)

    # Handle Kp0: If fixed, return a constant function; otherwise, fit it as a polynomial
    #if fixed_Kp0 is not None:
    #    fitted_funcs['Kp0'] = lambda t: fixed_Kp0  # Constant function returning fixed Kp0
    #else:
    #    popt, _ = curve_fit(quintic_function, Ts, Kp0s, maxfev=10000)
    #    fitted_funcs['Kp0'] = lambda t, popt=popt: quintic_function(t, *popt)

    if table:
        with open(table, 'w') as fh:
            for param in ['E0', 'V0', 'K0', 'Kp0']:
                if param == 'Kp0' and fixed_Kp0 is not None:
                    expr = f"{fixed_Kp0}"  # Just write the fixed value
                else:
                    popt = curve_fit(quintic_function, Ts, eval(f"{param}s"))[0]
                    expr = ' + '.join(
                        [f"{format_latex(coef, noplus=True)}T^{5 - i}" for i, coef in enumerate(popt[:5])])
...
def plot_eos_energy(
        ...
        parameters: BM3EOSParameters,
        T: float,
        fixed_Kp0: Optional[float] = None,  # Add fixed_Kp0 argument here
    ...
    # Use the fixed Kp0 if provided
    if fixed_Kp0 is not None:
        F_fit = bm3_eos_energy(V_fine, parameters.V0, parameters.E0, parameters.K0, fixed_Kp0)
    else:
...
def plot_twoplots(
        ...
        Ts: List[float],
        filename: Optional[str] = None,
        fixed_Kp0: Optional[float] = None  # Ensure this is passed
):
    ...
    for V, F, params, T in zip(vs, fs, eos_params, Ts):
        ...
        # Use fixed Kp0 if provided
        if fixed_Kp0 is not None:
            F_fit = bm3_eos_energy(V_fine, params.V0, params.E0, params.K0, fixed_Kp0)
        else:
            F_fit = bm3_eos_energy(V_fine, params.V0, params.E0, params.K0, params.Kp0)

        ax1.plot(V_fine, F_fit, color=color, linestyle='--')
    ...
    for params, T in zip(eos_params, Ts):
        ...
        # Use fixed Kp0 if provided
        if fixed_Kp0 is not None:
            P_fit = bm3_eos_pressure(V_fine, params.V0, params.K0, fixed_Kp0) * 160.218  # GPa
        else:
...
def main():
    ...
    parser.add_argument('--fixed_Kp0', type=float, default=None,
                        help='Fix the value of Kp0 during the fitting (optional)')
    ...
    eos_static = fit_eos(
        ...
        fixed_Kp0=args.fixed_Kp0,  # Pass fixed Kp0 here
    ...
    # In main()
    def fit_at_temperature(T, fixed_Kp0, verbose):
        if T == 'static':
        ...
        if args.use_pv and T == 'static':
            ...
            params = fit_eos(V, F, verbose=verbose, fixed_Kp0=fixed_Kp0)
        return (params, V, F)

    # Adjust Parallel call
    results = Parallel(n_jobs=-1)(
        delayed(fit_at_temperature)(T, args.fixed_Kp0, args.verbose) for T in unique_ts
    )
